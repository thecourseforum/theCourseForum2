...
from ..models import Answer, Question, Schedule, ScheduledCourse, User
...
# ---------------------------------------------------------------------------
# QA: Referer redirects and duplicate-check fallback
# ---------------------------------------------------------------------------


class QaRefererRedirectTestCase(TCFDataTestCase):
    """QA views bounce back to HTTP_REFERER or ``/``."""
    ...
    def test_new_question_post_valid_redirects_to_referer(self):
        """Successful question POST returns to the course page."""
        response = self.client.post(
            reverse("new_question"),
            {
                "text": "Will there be a curve?",
    ...
    def test_new_question_post_without_referer_uses_slash(self):
        """Missing Referer falls back to site root."""
        response = self.client.post(
            reverse("new_question"),
            {
    ...
    def test_new_answer_invalid_form_redirects_to_referer(self):
        """Validation errors still send the user back to Referer."""
        response = self.client.post(
            reverse("new_answer"),
            {},
            HTTP_REFERER=self.referer,
    ...
    def test_qa_check_duplicate_invalid_redirects_named_new_answer(self):
        """URL has no ``name=``; hit path directly."""
        response = self.client.post("/answers/check_duplicate/", {})
        self.assertRedirects(
            response,
            reverse("new_answer"),
            fetch_redirect_response=False,
...
class QaEditRefererRedirectTestCase(TCFDataTestCase):
    """Edit views always bounce to Referer (including GET)."""
    ...
    def setUp(self):
        ...
        self.referer = reverse(
            "course_instructor",
            args=[self.course.pk, self.instructor.pk],
        )

    def test_edit_question_get_redirects_to_referer(self):
        """GET never renders a form; it immediately returns to Referer."""
        response = self.client.get(
    ...
    def test_edit_question_post_valid_redirects_to_referer(self):
        """Successful edit returns to the prior page."""
        response = self.client.post(
            reverse("edit_question", args=[self.question.pk]),
            {
                "text": "Updated?",
    ...
    def test_edit_answer_post_valid_redirects_to_referer(self):
        """Answer edits use the same Referer pattern as questions."""
        ...
        response = self.client.post(
            reverse("edit_answer", args=[answer.pk]),
            {
                "text": "New body",
                "semester": str(self.semester.pk),
...
class QaDeleteSuccessUrlTestCase(TCFDataTestCase):
    """DeleteQuestion / DeleteAnswer success URLs point at course-instructor."""
    ...
    def test_delete_question_redirects_to_course_instructor(self):
        """Question delete success_url is the Q&A course page."""
        response = self.client.post(
    ...
    def test_delete_answer_redirects_to_course_instructor(self):
        """Answer delete returns to the same course-instructor URL."""
        answer = Answer.objects.create(
            text="A.",
            question=self.question,
            user=self.user1,
        ...
        response = self.client.post(reverse("delete_answer", args=[answer.pk]))
        expected = reverse(
            "course_instructor",
            args=[self.course.pk, self.instructor.pk],
        )
