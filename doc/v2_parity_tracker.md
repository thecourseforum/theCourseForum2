...
Last updated: 2026-02-07 (post parity re-audit)  
...
- Landing-page CTA section ("Ready to explore?") is intentionally removed in v2.
...
6. Feedback parity: post-action success/error/info messages render on every v2 page via shared base layout.

## Route Coverage Matrix
| Legacy Route | V2 Route | Status | Notes |
| --- | --- | --- | --- |
| `/` | `/v2/` | Done | Core landing parity for approved scope (hero/search/features/FAQ); legacy-only landing sections were intentionally retired. |
...
| --- | --- | --- | --- | --- |
| Landing | `tcf_website/templates/landing/landing.html` | `tcf_website/templates/v2/pages/landing.html` | Done | Hero/search/features/FAQ are in place; legacy-only blocks are intentionally retired and CTA section has been removed by request. |
...
| Course Instructor | `tcf_website/templates/course/course_professor.html` | `tcf_website/templates/v2/pages/course_instructor.html` | Done | Q&A and instructor switcher intentionally removed; workload + weekly summary added; sections split into lecture/other with in-panel scrolling; sort uses in-place update with `#reviews`. |
...
| Add Course to Schedule | legacy schedule modal flow from course page | `tcf_website/templates/v2/pages/schedule_add_course.html` | Done | Dedicated v2 flow supports both lecture and other section types, keeps options scrollable, and enforces duplicate/conflict checks server-side. |
...
| `course_view` / `course_view_v2` | Done | `tcf_website/views/browse.py:348`, `tcf_website/views/browse.py:453` | Same core course/instructor retrieval with approved v2 presentation changes (rating/GPA-only cards and lecture-only time chips). |
| `course_instructor` / `course_instructor_v2` | Done | `tcf_website/views/browse.py:522`, `tcf_website/views/browse.py:685` | Core review/grade/section logic preserved; workload breakdown and grade summary visibility implemented; approved divergences documented. |
...
| `view_schedules` / `view_schedules_v2` | Partial | `tcf_website/views/schedule.py` | Shared schedule aggregates preserved; v2 uses reusable calendar/context and simplified form actions; schedule-page search/autocomplete remains deferred. |
| Course-page add flow / `schedule_add_course_v2` | Done | `tcf_website/views/schedule.py` | Login-protected v2 add flow includes lecture + other sections, scrollable section chooser, duplicate/conflict checks, and validation-safe error handling. |
...
- Verified v2 base-level flash messages: all v2 pages now render `messages` consistently after redirects/actions.
...
- [x] Standardize v2 message rendering in shared layout to avoid delayed/stale flash messages.
- [ ] Add schedule-page course search/autocomplete (planned shared component with v2 new-review; currently excluded from this phase).
...
- [x] Remove landing CTA section and unused styling.
...
- [ ] `v2` course -> add-to-schedule flow adds lecture/other section selections and handles conflict/duplicate/error states.
...
- [ ] Flash messages appear immediately after v2 mutations (create/edit/delete/add/remove) without delayed carry-over.
...
- 2026-02-07: Added weekly-hours summary display in Ratings panel ("Weekly Breakdown" header row).
...
- 2026-02-07: Added dedicated v2 add-to-schedule flow from course page with lecture + other section groups, server-side conflict checks, and scrollable chooser.
- 2026-02-07: Standardized v2 flash-message rendering in `v2/base.html` and removed page-local duplication.
- 2026-02-07: Removed landing-page CTA section ("Ready to explore?") and corresponding v2 CSS.
