document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".search-bar-container").forEach((container) => {
    ...
    const autocompleteContainer = container.querySelector('.autocomplete-dropdown-container');
    ...
    if (searchInput && autocompleteContainer) {
      ...
      searchInput.addEventListener('focus', () => {
        if (searchInput.value.trim().length >= 2 && autocompleteContainer.innerHTML.trim() !== "") {
          autocompleteContainer.style.display = 'block';
        }
      });
      ...
      searchInput.addEventListener('keydown', (e) => {
        ...
        const items = Array.from(autocompleteContainer.querySelectorAll('.autocomplete-item__link, .autocomplete-item__title')); // if no link, select title
        if (!items.length) return;
        
        let activeIdx = items.findIndex(item => document.activeElement === item);
        ...
        } else if (document.activeElement !== searchInput && (e.key === 'Enter')) {
      ...
      autocompleteContainer.addEventListener('keydown', (e) => {
        ...
        const items = Array.from(autocompleteContainer.querySelectorAll('.autocomplete-item__link'));
        let activeIdx = items.findIndex(item => document.activeElement === item);
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          if (activeIdx < items.length - 1) items[activeIdx + 1].focus();
